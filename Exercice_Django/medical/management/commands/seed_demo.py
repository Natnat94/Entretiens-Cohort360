...
def random_date_from_starting_date(starting_date, days_range=1000):
    return starting_date + timedelta(days=random.randint(1, days_range))
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--prescriptions", type=int, default=35)
    ...
    def handle(self, *args, **options):
        ...
        for i in range(n_prescriptions):
            ...
            p = Prescription.objects.create(
                ...
                ending_date=random_date_from_starting_date(starting_date),
                status=random.choices(
                    [Prescription.STATUS_ACTIF, Prescription.STATUS_SUPPR, Prescription.STATUS_PENDING],
                    weights=[0.6, 0.2, 0.2],
                )[0],
            )
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Created {len(created_patients)} patients, {len(created_meds)} medications and {len(created_prescriptions)} prescriptions."
            )
        )
